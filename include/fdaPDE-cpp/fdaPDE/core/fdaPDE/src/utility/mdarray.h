...
// required by MVSC
template <int N> struct make_full_dynamic_extents {
    static constexpr auto value() {
        return internals::apply_index_pack<N>([]<int... Ns_> { return MdExtents<((void)Ns_, Dynamic)...> {Ns_...}; });
    }
    using type = decltype(value());
};

template <int N> using full_dynamic_extent_t = typename make_full_dynamic_extents<N>::type;
