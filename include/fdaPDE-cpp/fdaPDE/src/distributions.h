...
struct bernoulli_distribution : public internals::distribution_base<std::bernoulli_distribution> {
   ...
   public:
    ...
    result_type cdf(double x) const { return x < 0 ? 0 : ((x >= 0 && x < 1) ? 1 - p_ : 1); }
    result_type mean() const { return p_; }
    result_type variance() const { return p_ * (1 - p_); }
    result_type quantile(double alpha) const {
    ...
    template <typename T> const T& mean(const T& data) const { return data; }
    template <typename T> requires(!internals::is_eigen_dense_xpr_v<T>) auto variance(const T& data) const {
        return Base::apply_(data, [](auto v) { return v * (1 - v); });
    }
    template <typename T> requires(!internals::is_eigen_dense_xpr_v<T>) auto link(const T& data) const {
        return Base::apply_(data, [](auto v) { return std::log(v / (1 - v)); });
    }
    template <typename T> requires(!internals::is_eigen_dense_xpr_v<T>) auto inv_link(const T& data) const {
        return Base::apply_(data, [](auto v) { return 1.0 / (1 + std::exp(-v)); });
    }
    template <typename T> requires(!internals::is_eigen_dense_xpr_v<T>) auto der_link(const T& data) const {
        return Base::apply_(data, [](auto v) { return 1.0 / (v * (1 - v)); });
    }
    template <typename T>
        requires(std::is_floating_point_v<T>)
    result_type deviance(T x, T y) const {
...
struct rademacher_distribution : public bernoulli_distribution {
   ...
   public:
    ...
    // density function
    template <typename InputType> result_type pdf(InputType x) const {
        return (x == 1 || x == -1) ? 0.5 : 0.0;
    }
    result_type cdf(double x) const { return x < -1 ? 0 : ((-1 <= x && x < 1) ? 0.5 : 1.0); }
    result_type mean() const { return 0.0; }
    result_type variance() const { return 1.0; }
    // random sampling
...
struct poisson_distribution : public internals::distribution_base<std::poisson_distribution<int>> {
   ...
   public:
    ...
    result_type pdf(InputType k) const {
        return std::pow(l_, k) * std::exp(-l_) / factorial(k);
    }
    result_type cdf(double x) const {
        double cdf_ = 0;
        for (int i = 0; i < std::floor(x); ++i) { cdf_ += std::pow(l_, i) / factorial(i); }
        return cdf_ * std::exp(-l_);
    }
    result_type mean() const { return l_; }
    result_type variance() const { return l_; }
    result_type quantile(double alpha) const {
        fdapde_assert(alpha >= 0 && alpha <= 1);
        int x = 0;
        double cdf_ = 0;
    ...
    template <typename T> const T& mean(const T& data) const { return data; }
    template <typename T> const T& variance(const T& data) const { return data; }
    template <typename T> requires(!internals::is_eigen_dense_xpr_v<T>) auto link(const T& data) const {
...
struct exponential_distribution : public internals::distribution_base<std::exponential_distribution<double>> {
   ...
   public:
    ...
    result_type cdf(double x) const { return 1.0 - std::exp(-l_ * x); }
    result_type mean() const { return 1.0 / l_; }
    result_type variance() const { return 1.0 / (l_ * l_); }
    result_type quantile(double alpha) const { return -std::log(1 - alpha) / l_; }
    // random sampling
    template <typename RandomNumberGenerator> result_type operator()(RandomNumberGenerator& rng) { return distr_(rng); }

    template <typename T> requires(!internals::is_eigen_dense_xpr_v<T>) const T& mean(const T& data) const {
        return Base::apply_(data, [](auto v) { return 1.0 / v; });
    }
    template <typename T>
        requires(!internals::is_eigen_dense_xpr_v<T>)
    const T& variance(const T& data) const {
        return Base::apply_(data, [](auto v) { return v * v; });
    }
    template <typename T> auto link(const T& data) const {
        return Base::apply_(data, [](auto v) { return -1.0 / v; });
    }
    template <typename T> auto inv_link(const T& data) const {
        return Base::apply_(data, [](auto v) { return -1.0 / v; });
    }
    template <typename T> auto der_link(const T& data) const {
...
class gamma_distribution : public internals::distribution_base<std::gamma_distribution<double>> {
   ...
   public:
    ...
    result_type mean() const { return k_ * theta_; }
    result_type variance() const { return k_ * theta_ * theta_; }
    // random sampling
    template <typename RandomNumberGenerator> result_type operator()(RandomNumberGenerator& rng) { return distr_(rng); }
  
    template <typename T> const T& mean(const T& data) const {
        return Base::apply_(data, [](auto v) { return v; });
    }
    template <typename T> const T& variance(const T& data) const {
        return Base::apply_(data, [](auto v) { return v * v; });
    }
    template <typename T> auto link(const T& data) const {
        return Base::apply_(data, [](auto v) { return -1.0 / v; });
...
class normal_distribution : public internals::distribution_base<std::normal_distribution<double>> {
  ...
  private:
    ...
    param_type sigma_;
   public:
    normal_distribution() noexcept = default;
    normal_distribution(param_type mu, param_type sigma) : Base(mu, sigma), mu_(mu), sigma_(sigma) { }
    // density function
    ...
    param_type mean() const { return mu_; }
    param_type variance() const { return sigma_ * sigma_; }
...
class chi_squared_distribution : public internals::distribution_base<std::chi_squared_distribution<double>> {
   ...
   public:
    chi_squared_distribution() noexcept = default;
    chi_squared_distribution(param_type n) : n_(n), gamma_(n / 2, 2) { }
    chi_squared_distribution(param_type n, param_type s) :
    ...
    result_type mean() const { return n_; }
    result_type variance() const { return 2 * n_; }
    ...
    param_type dofs() const { return n_; }
    // random sampling
    template <typename RandomNumberGenerator> result_type operator()(RandomNumberGenerator& rng) { return distr_(rng); }

    template <typename T> const T& mean(const T& data) const { return gamma_.mean(data); }
    template <typename T> const T& variance(const T& data) const { return gamma_.variance(data); }
    template <typename T> auto link(const T& data) const { return gamma_.link(data); }
    template <typename T> auto inv_link(const T& data) const { return gamma_.inv_link(data); }
    template <typename T> auto der_link(const T& data) const { return gamma_.der_link(data); }
    template <typename T>
        requires(std::is_floating_point_v<T>)
    result_type deviance(T x, T y) const {
