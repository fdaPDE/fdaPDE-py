...
template <typename Triangulation> class GeoFrame {
   ...
   public:
    ...
    void
    insert_scalar_point_layer(const std::string& layer_name, const GeoDescriptor& locs, const pybind11::dict& data) {
        ...
        auto copy_ = [&]<typename T>(const std::string& field) {
            if (data.contains(field)) {
                pybind11::dict dct = pybind11::cast<pybind11::dict>(data[field.data()]);
                for (const auto& item : dct) {
                    std::string name = pybind11::cast<std::string>(item.first);
                    pybind11::list py_data = pybind11::cast<pybind11::list>(item.second);
                    // convert into a cpp vector
                    std::vector<T> vec;
                    vec.reserve(py_data.size());
                    for (auto v : py_data) { vec.push_back(pybind11::cast<T>(v)); }
                    // load data
                    l.load_vec(name, vec);
                }
            }
    ...
    void insert_scalar_areal_layer(
      const std::string& layer_name, const std::vector<int>& regions, const pybind11::dict& data) {
    ...
    // areal layer
    std::vector<pybind11::dict> areal_polygons(const std::string& layer_name) {
        ...
        std::vector<pybind11::dict> polygons;
    ...
    const geoframe_t& data() const { return data_; }
   private:
    geoframe_t data_;
};
